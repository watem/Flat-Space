pub mod canvas {
    ...
    use crate::model::real_space::physics::{Body, XY};
    use crate::view::{View};
    ...
    impl View
    {
        pub fn draw_point_demo(&self, pos: &XY, size: f32, color: Option<Color>)
        {
            ...
            let mid_x: f32 = screen_width() / 2.0 - self.get_system().centre().get_x() as f32;
            let mid_y: f32 = screen_height() / 2.0 + self.get_system().centre().get_y() as f32;

            let x = mid_x + (pos.get_x() * self.get_system().zoom) as f32;
            let y = mid_y + (pos.get_y() * self.get_system().zoom) as f32;
    ...
    pub fn draw(body: &Body) {
        ...
        let x = mid_x + body.pos.get_x() as f32;
        let y = mid_y + body.pos.get_y() as f32;
    ...
    pub fn draw_point(xy: &XY, size: f32, color: Color, scale: f64) {
        ...
        let x = mid_x + (xy.get_x() * scale) as f32;
        let y = mid_y + (xy.get_y() * scale) as f32;
        draw_circle(x, y, size, color);
    }
